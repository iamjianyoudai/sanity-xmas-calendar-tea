        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-5xl font-bold text-green-800">{tea.name}</h1>
              {tea.category && (
                <span
                  className="px-4 py-2 bg-green-100 text-green-700 rounded-full text-lg font-semibold"
                  style={
                    tea.category.color
                      ? {
                          backgroundColor: tea.category.color + "20",
                          color: tea.category.color,
                        }
                      : {}
                  }
                >
            ...
            {tea.origin && (
              <p className="text-lg text-gray-600">Origin: {tea.origin}</p>
            )}
          </div>

          {/* Image Section */}
          {(() => {
            const localImage = getTeaImage(tea.name, tea.slug.current);
            ...
            <div className="bg-white rounded-lg shadow-md p-8 mb-8">
              ...
              <div className="flex flex-wrap gap-3">
                {tea.flavorNotes.map((note, index) => (
                  <span
                    key={index}
                    className="px-4 py-2 bg-green-50 text-green-700 rounded-full text-base font-medium border border-green-200"
                  >
                    {note}
                  </span>
                ))}
